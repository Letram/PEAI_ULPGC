...
//  DeviceMotion
//
//  Created by Zenón J. Hernández Figueroa on 27/4/16.
//  Copyright © 2016 Zenón J. Hernández Figueroa. All rights reserved.
//
...
class ViewController: UIViewController {
    ...
    override func viewDidLoad() {
        ...
        if self.motionManager.isDeviceMotionAvailable {
            ...
            self.motionManager.startDeviceMotionUpdates(using: .xArbitraryZVertical, to: self.operationQueue, withHandler: { (data: CMDeviceMotion?, error: Error?) in
                if error == nil {
                    self.newValues = self.updatePosition(xValue: CGFloat((data?.attitude.roll)!) ,yValue: CGFloat((data?.attitude.pitch)!))
    ...
    func updatePosition(xValue: CGFloat, yValue: CGFloat) -> [CGFloat]{
